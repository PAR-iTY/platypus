import { isMicSupported } from './feature-detection/isMicSupported.js';
...
var Main = (function () {
  ...
  var blobs = [], // array to hold latest recording for the session
    ...
    featuresResults; // results from feature detection module
  ...
  function init() {
    ...
    try {
      ...
      alert(
        'Web Audio API not supported, please try updating or switching browsers to continue. Error: ',
        e
      );
    ...
    try {
      ...
      alert(
        'Recorder not supported, please try updating or switching browsers to continue. Error: ',
        e
      );
      window.dispatchEvent(new Event('unload'));
      return;
    ...
    $('#source')
      ...
      .on('error', function (e) {
        console.log('durationchange error');
        alert('media error: ' + e.code + ': ' + e.message);
        window.dispatchEvent(new Event('unload'));
        return;
      });
  ...
  var MP3Recorder = function (config) {
    ...
    config = config || {};
    var realTimeWorker = new Worker('./dependencies/worker-realtime.js');
    ...
    realTimeWorker.onmessage = function (e) {
      switch (e.data.cmd) {
        ...
        default:
          alert(
            'Web Worker received a message it does not know how to handle: ' +
              e.data
          );
      }
    };
  ...
  $('#startBtn').on('click', function (e) {
    e.preventDefault();
    // run feature test module on user click
    // check if results already exist
    if (!featuresResults) {
      featuresResults = isMicSupported();
      // async not handled for featuresResults.support yet
      // will will still appear in log after a split second or so
      console.log(featuresResults);
    }
    ...
    recorder.start(
      ...
      function (e) {
        ...
        alert(
          e,
          'Could not make use of your microphone, please check your hardware is working:'
        );
        window.dispatchEvent(new Event('unload'));
        return;
      }
  ...
  $('#stopBtn').on('click', function (e) {
    ...
    // reveal UI elements
    $('#slide-wrap, #slider, #playerUI, #storeBtn, #upBtn, #editBtn').css(
      'visibility',
      'visible'
    );
  });
  ...
  window.addEventListener('beforeunload', function (e) {
    ...
    if (blobs.length !== 0 && saveCount === 0) {
      var confirmationMessage =
        'Are you sure you want to leave? Any unsaved recordings will be lost';
